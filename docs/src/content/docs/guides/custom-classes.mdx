...
Every entry in `den.classes.<name>` is a record of options, not just a marker:

| Field | Type | Default | Description |
|---|---|---|---|
| `description` | `str` | -- | Human-readable description of the class domain |
| `forwardTo` | `nullOr { class; path? }` | `null` | Default routing target for the `forward` battery: when a forward omits `intoClass`/`intoPath`, the class's `forwardTo.class` and `forwardTo.path` are used instead |
| `parentPath` | `nullOr (name -> path)` | `null` | For classes whose members nest inside an enclosing config owner (e.g. home-manager inside a host): a function `name -> attrpath` locating a named member within that owner's config -- the same route the class's content is delivered to. `null` for root classes that own a top-level config (`nixos`, `darwin`, …) |
| `parentArg` | `nullOr str` | `null` | For nested classes, the module argument by which a member reaches the enclosing config owner (home-manager exposes the host config as `osConfig`). `null` for root classes |

`forwardTo` lets a class declare its destination once, so forwards only name
the source:
...
`parentPath`/`parentArg` are what make **nested** classes work -- the pipe
layer uses them to resolve a producer's config at its producing class +
scope. Den's own `homeManager` class registers both:

```nix
den.classes.homeManager.parentPath = userName: [ "home-manager" "users" userName ];
...
| `adaptArgs = args: attrs` | Set module arguments for the forwarded source |
...
Config guards read the final target config. The value must have a definition
that does not depend on the same guarded forward. A self-dependent guard has
normal Nix module recursion semantics.

</Aside>

For a nested forward, Den gives the source evaluator its own `config`,
`options`, and `lib` arguments when `adaptArgs` is absent. Other module
arguments, such as `pkgs`, remain available.

Den uses an explicit `adaptArgs` result as `specialArgs`. Reserved names in the
result override the source arguments. Use a named argument when the source needs
the target config:
...
The source can then request `config` for its local configuration and
`osConfig` for the target configuration.
